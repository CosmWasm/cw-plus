...
const CW1 = (client: SigningCosmWasmClient, options: Options): CW1Contract => {
  const use = (contractAddress: string): CW1Instance => {
    ...
    const increaseAllowance = async (senderAddress: string, spender: string, amount: Coin, expires?: Expiration): Promise<string> => {
      const fee = calculateFee(options.fees.exec, options.gasPrice);

      const result = await client.execute(senderAddress, contractAddress, { increase_allowance: { spender, amount, expires } }, fee);
      return result.transactionHash;
    };

    const decreaseAllowance = async (senderAddress: string, spender: string, amount: Coin, expires?: Expiration): Promise<string> => {
      const fee = calculateFee(options.fees.exec, options.gasPrice);

      const result = await client.execute(senderAddress, contractAddress, { decrease_allowance: { spender, amount, expires } }, fee);
      return result.transactionHash;
    };

    const setPermissions = async (senderAddress: string, spender: string, permissions: Permissions): Promise<string> => {
      const fee = calculateFee(options.fees.exec, options.gasPrice);

      const result = await client.execute(senderAddress, contractAddress, { set_permissions: { spender, permissions } }, fee);
      return result.transactionHash;
    };
