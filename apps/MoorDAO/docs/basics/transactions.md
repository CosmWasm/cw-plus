...
Aragon Chain needs to parse and handle transactions routed for both the EVM and for the Cosmos hub. We
...
Being that Aragon Chain implements the Tendermint ABCI application interface, as transactions are
...
internal interaction between Aragon Chain and Tendermint will utilize one of the supported encoding
...
Aragon Chain supports [EIP-155](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md)
signatures. A `Transaction` is expected to have a single signature for Ethereum routed transactions.
However, just as in Cosmos, Aragon Chain will support multiple signers for non-Ethereum transactions.
...
Learn about how [gas](./gas.md) is used on Aragon Chain {hide}
